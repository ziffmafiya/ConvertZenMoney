...
class TheilSenRegression extends mlRegressionBase.BaseRegression {
  /**
   * Theil–Sen estimator
  ...
  toString(precision) {
    let result = 'f(x) = ';
    if (this.slope) {
      let xFactor = mlRegressionBase.maybeToPrecision(this.slope, precision);
      result += `${Math.abs(xFactor - 1) < 1e-5 ? '' : `${xFactor} * `}x`;
      if (this.intercept) {
        let absIntercept = Math.abs(this.intercept);
        let operator = absIntercept === this.intercept ? '+' : '-';
        result += ` ${operator} ${mlRegressionBase.maybeToPrecision(absIntercept, precision)}`;
      }
    } else {
      result += mlRegressionBase.maybeToPrecision(this.intercept, precision);
    }
    return result;
  }
