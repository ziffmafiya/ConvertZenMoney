...
function oplsNipals(data, labels, options = {}) {
    ...
    data = ml_matrix_1.Matrix.checkMatrix(data);
    labels = ml_matrix_1.Matrix.checkMatrix(labels);
    let tW = [];
    if (labels.columns > 1) {
        ...
        do {
            if (count === 0) {
                pcaW = new ml_matrix_1.NIPALS(wh.clone());
                tW.push(pcaW.t);
            }
            else {
                const data = pcaW.xResidual;
                pcaW = new ml_matrix_1.NIPALS(data);
                tW.push(pcaW.t);
            }
            ssT = pcaW.t.norm() ** 2;
            count++;
...
function getWh(xValue, yValue) {
    let result = new ml_matrix_1.Matrix(xValue.columns, yValue.columns);
