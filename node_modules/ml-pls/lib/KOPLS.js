...
class KOPLS {
    ...
    constructor(options, model) {
        if (options === true) {
            this.trainingSet = new ml_matrix_1.Matrix(model.trainingSet);
            this.YLoadingMat = new ml_matrix_1.Matrix(model.YLoadingMat);
            this.SigmaPow = new ml_matrix_1.Matrix(model.SigmaPow);
            this.YScoreMat = new ml_matrix_1.Matrix(model.YScoreMat);
            this.predScoreMat = (0, utils_1.initializeMatrices)(model.predScoreMat, false);
            this.YOrthLoadingVec = (0, utils_1.initializeMatrices)(model.YOrthLoadingVec, false);
            this.YOrthEigen = model.YOrthEigen;
            this.YOrthScoreMat = (0, utils_1.initializeMatrices)(model.YOrthScoreMat, false);
            this.toNorm = (0, utils_1.initializeMatrices)(model.toNorm, false);
            this.TURegressionCoeff = (0, utils_1.initializeMatrices)(model.TURegressionCoeff, false);
            this.kernelX = (0, utils_1.initializeMatrices)(model.kernelX, true);
    ...
    train(trainingSet, trainingValues) {
        ...
        let Identity = ml_matrix_1.Matrix.eye(kernelX.rows, kernelX.rows, 1);
        let temp = kernelX;
        ...
        kernelX[0][0] = temp;
        let result = new ml_matrix_1.SingularValueDecomposition(trainingValues.transpose().mmul(kernelX[0][0]).mmul(trainingValues), {
        ...
        let SigmaPow = ml_matrix_1.Matrix.pow(Sigma, -0.5);
        ...
        for (let i = 0; i < this.orthogonalComp; ++i) {
            ...
            TURegressionCoeff[i] = (0, ml_matrix_1.inverse)(TpiPrime.mmul(predScoreMat[i]))
                .mmul(TpiPrime)
                .mmul(YScoreMat);
            result = new ml_matrix_1.SingularValueDecomposition(TpiPrime.mmul(ml_matrix_1.Matrix.sub(kernelX[i][i], predScoreMat[i].mmul(TpiPrime))).mmul(predScoreMat[i]), {
                computeLeftSingularVectors: true,
                computeRightSingularVectors: false,
            });
            ...
            YOrthScoreMat[i] = ml_matrix_1.Matrix.sub(kernelX[i][i], predScoreMat[i].mmul(TpiPrime))
                .mmul(predScoreMat[i])
                .mmul(YOrthLoadingVec[i])
                .mul(Math.pow(YOrthEigen[i], -0.5));
            let toiPrime = YOrthScoreMat[i].transpose();
            YOrthScoreNorm[i] = ml_matrix_1.Matrix.sqrt(toiPrime.mmul(YOrthScoreMat[i]));
            YOrthScoreMat[i] = YOrthScoreMat[i].divRowVector(YOrthScoreNorm[i]);
            let ITo = ml_matrix_1.Matrix.sub(Identity, YOrthScoreMat[i].mmul(YOrthScoreMat[i].transpose()));
        ...
        TURegressionCoeff[this.orthogonalComp] = (0, ml_matrix_1.inverse)(lastTpPrime.mmul(lastScoreMat))
            .mmul(lastTpPrime)
            .mmul(YScoreMat);
    ...
    predict(toPredict) {
        ...
        for (i = 0; i < this.orthogonalComp; ++i) {
            predScoreMat[i] = KTestTrain[i][0]
                .mmul(this.YScoreMat)
                .mmul(this.SigmaPow);
            YOrthScoreVector[i] = ml_matrix_1.Matrix.sub(KTestTrain[i][i], predScoreMat[i].mmul(this.predScoreMat[i].transpose()))
            ...
            KTestTrain[i + 1][0] = ml_matrix_1.Matrix.sub(KTestTrain[i][0], YOrthScoreVector[i]
                .mmul(scoreMatPrime)
                .mmul(this.kernelX[0][i].transpose()));
            let p1 = ml_matrix_1.Matrix.sub(KTestTrain[i][0], KTestTrain[i][i].mmul(this.YOrthScoreMat[i]).mmul(scoreMatPrime));
