...
exports.xyReduceNonContinuous = xyReduceNonContinuous;
const zones_1 = require("../zones");
const xyCheck_1 = require("./xyCheck");
const xyReduce_1 = require("./xyReduce");
/**
...
function xyReduceNonContinuous(data, options = {}) {
    ...
    let { zones = [] } = options;
    zones = (0, zones_1.zonesNormalize)(zones, { from, to });
    if (zones.length === 0)
        zones = [{ from, to }]; // we take everything
    const { internalZones, totalPoints } = (0, xyReduce_1.getInternalZones)(zones, x);
    // we calculate the number of points per zone that we should keep
    if (totalPoints <= maxApproximateNbPoints) {
        return (0, xyReduce_1.notEnoughPoints)(x, y, internalZones, totalPoints);
    }
...
//# sourceMappingURL=xyReduceNonContinuous.js.map