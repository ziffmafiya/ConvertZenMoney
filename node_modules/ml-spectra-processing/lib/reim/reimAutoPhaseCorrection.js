...
function reimAutoPhaseCorrection(data, options = {}) {
    ...
    const phased = (0, reimPhaseCorrection_1.reimPhaseCorrection)({ re, im }, toRadians(ph0), toRadians(ph1), { reverse });
...
function detectBaselineRegions(data, options) {
    const magnitudeData = options.magnitudeMode ? (0, reimAbsolute_1.reimAbsolute)(data) : data.re;
...
function autoPhaseRegion(re, im, x0) {
    ...
    while (maxSteps > 0) {
        ...
        for (let i = start; i <= stop; i += dAng) {
            const tmpPhased = (0, reimPhaseCorrection_1.reimPhaseCorrection)({ re, im }, toRadians(i), 0);
    ...
    const phased = (0, reimPhaseCorrection_1.reimPhaseCorrection)({ re, im }, toRadians(bestAng), 0);
    let area = 0;
    let sumX = 0;
...
function robustBaseLineRegionsDetection(s, options) {
    ...
    let change = true;
    while (change) {
        const noiseLevel = (0, x_1.xNoiseSanPlot)(s, { magnitudeMode });
...
//# sourceMappingURL=reimAutoPhaseCorrection.js.map