...
// Если ключ есть, инициализируем модели ИИ для работы с эмбеддингами
 ...
 * Включает дедупликацию, фильтрацию и генерацию эмбеддингов
...
export default async function handler(req, res) {
    ...
    // Валидация входных данных - проверяем наличие и формат транзакций
    ...
    // Проверяем, что все необходимые ключи конфигурации заданы
    ...
    // Инициализируем клиент Supabase для работы с базой данных
    const supabase = createClient(supabaseUrl, supabaseKey);

    /**
     * Нормализует строку: удаляет неразрывные пробелы, обрезает пробелы по краям и приводит к нижнему регистру
     * Используется для консистентного сравнения строк при фильтрации
     ...
     * Это позволяет эффективно идентифицировать дубликаты в базе данных
    ...
    const createUniqueHash = (t) => {
        ...
        // Безопасно обрабатывает null/undefined значения и обеспечивает согласованное форматирование чисел
        ...
        // Очищаем и нормализуем текстовые поля
    ...
    try {
        // Шаг 1: Генерируем хэши для всех входящих транзакций для последующей дедупликации
        ...
        // Шаг 3: Отфильтровываем транзакции, которые уже есть в базе данных (дедупликация)
